<template>
  <div class="product">
    <v-popup v-if="isProductInfoVisible" @closePopup="closePopup" :partDetail="partDetail" />
    <div class="image"><img alt="photo" :src="'data:image/png;base64,' + product_data.mainPhoto"></div>
    <div class="product-footer">
      <div v-if="product_data.price !== 0" class="price">
        <p class="text">Цена: {{ product_data.price }}</p>
      </div>
      <div class="description">
        <p class="text">Винкод: {{ product_data.winCode }}</p>
      </div>
      <div class="description">
        <p class="text">{{ product_data.title }}</p>
      </div>
      <div class="button-block">
        <a @click="counter"
          :href="`https://wa.me/77078276330?text=Здравствуйте,%20по%20поводу%20детали%20${product_data.title}%20код%20детали:%20${product_data.winCode}`"
          class="buy-button">Заказать</a>
        <button class="product-button" @click="showProductInfo">
          Информация
        </button>
      </div>
    </div>
